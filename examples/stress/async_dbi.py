"""Asynchronous database interface for the stress example.

Mirror of sync_dbi but using AsyncFunctionBinder, exercising: async
...
async def create_accounts_table(pk_col_type: str):
...
async def create_transfers_table(pk_col_type: str):
    """Create the transfers table."""
    pass


@binder.transaction()
async def init_schema(pk_col_type: str):
    """Drop and recreate all tables inside a single transaction."""
    await drop_transfers_table()
    await drop_accounts_table()
    await create_accounts_table(pk_col_type)
    await create_transfers_table(pk_col_type)
...
async def get_account_by_name(name: str, row_lock: str = "") -> Optional[Account]:
...
async def stream_transfers_for(
    account_id: int,
) -> AsyncGenerator[Transfer, None]:
...
async def transfer(from_name: str, to_name: str, amount: int) -> dict:
    ...
    """
    src = await get_account_by_name(from_name, row_lock=ROW_LOCK)
    dst = await get_account_by_name(to_name, row_lock=ROW_LOCK)
